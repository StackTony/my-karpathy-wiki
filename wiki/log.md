...
## [2026-05-16] config | Wiki 工作流技能创建

**新增技能（3个）**：
- `/自建更新`：手动触发 Ingest 工作流，检查 raw/sources 变化并生成摘要
- `/自检索引`：手动触发 Lint 工作流，检查断链、孤立页面、索引一致性
- `/自学推荐`：手动触发 Learn 工作流，生成每日学习推荐报告

**技能位置**：`.claude/skills/wiki-workflows/`
