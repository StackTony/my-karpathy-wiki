...
|架构|数据面经 QEMU|数据面经 KVM|数据面位置|上下文切换|性能|硬件要求|
|---|---|---|---|---|---|---|
|**传统 virtio**|✅ 经过|✅ 经过|QEMU 用户态|最多|⭐ 低|无|
|**vhost-net**|❌ 绕过|✅ 经过|内核态|中等|⭐⭐ 中|无|
|**vhost-user**|❌ 绕过|✅ 经过|独立用户态后端|较少|⭐⭐⭐ 高|无特殊要求|
|**vDPA**|❌ 绕过|✅ 经过|硬件|最少|⭐⭐⭐⭐ 最高|需要 vDPA 硬件|
#### 一、virtio-net 设备
...
| 数据面经过 QEMU | 数据面经过 KVM | 说明              |
| ---------- | --------- | --------------- |
| ✅ **经过**   | ✅ **经过**  | 数据包完整经过用户态 QEMU |
